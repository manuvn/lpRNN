...
from model.RNNCNN import RNNCNN, load_model
from math import exp
from lprnn_keras_impl import lpRNN
from support import plot_history
...
def main(opt):
    ...
    if not opt.skip_train:
        print('creating an RNN-CNN with', opt.num_layers, 'layers')
        model = RNNCNN(opt)
        ...
        history = model.fit_generator(loader.next_batch(Train), loader.split_sizes[Train], opt.max_epochs,
...
if __name__=="__main__":
    ...
    # model params
    parser.add_argument('--rnn_size', type=int, default=650, help='size of RNN internal state')
    parser.add_argument('--rnn_type', type=str, default='lpLSTM', help='type of RNN model')
    ...
    path = './results/penntree/'

    history = main(params)
    history.history['perplexity']    = np.exp(history.history['loss'])

    fname = 'lpLSTM.hist'
    fhandle = open(fname, 'wb')
    pickle.dump(history, fhandle)
    fhandle.close()