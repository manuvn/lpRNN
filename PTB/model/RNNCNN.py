...
from keras.layers import Input, Embedding, TimeDistributed, Dense, Dropout, Reshape, Concatenate, LSTM, CuDNNLSTM, Conv2D, MaxPooling2D, BatchNormalization,GRU, SimpleRNN
...
class pplHistory(Callback):

    def on_epoch_end(self, epoch, logs={}):
        perplexity = exp(logs.get('val_loss'))
...
class sModel(Model):
    def fit_generator(self, generator, steps_per_epoch, epochs, validation_data, validation_steps, opt):
        ...
        history = super(sModel, self).fit_generator(generator, 
                       steps_per_epoch, 
                       epochs=epochs,
                       verbose=1,
                       validation_data=validation_data,
                       validation_steps = validation_steps,
                       callbacks=[reduce_lr, myppl])
#        val_loss = exp(self.evaluate_generator(validation_data, validation_steps))
        
        
#        for epoch in range(epochs):
#            history = super(sModel, self).fit_generator(generator, steps_per_epoch, epochs=epoch+1, verbose=1, initial_epoch=epoch)
#            val_loss = exp(self.evaluate_generator(validation_data, validation_steps))
...
#            print('Current learning rate ', self.optimizer.lr)
...
def RNNCNN(opt):
    ...
    # opt.rnn_type = RNN type
    RNN_model = eval(opt.rnn_type)    
    if opt.use_words:
    ...
    for l in range(opt.num_layers):
        print(opt.rnn_type, type(opt.rnn_type))
        if opt.rnn_type == 'CuDNNLSTM':
            x = RNN_model(opt.rnn_size, return_sequences=True, stateful=True)(x)            
        elif opt.rnn_type == 'LSTM':
            x = RNN_model(opt.rnn_size, activation=opt.activation, recurrent_activation='sigmoid', return_sequences=True, stateful=True)(x)
        elif opt.rnn_type == 'lpLSTM':
            x = RNN_model(opt.rnn_size, activation=opt.activation, learn_retention_ratio=False, recurrent_activation='sigmoid', return_sequences=True, stateful=True)(x)
        elif opt.rnn_type == 'lpRNN':
            x = RNN_model(opt.rnn_size, activation=opt.activation, learn_retention_ratio=False, return_sequences=True, stateful=True)(x)
        else:
            x = RNN_model(opt.rnn_size, activation=opt.activation, return_sequences=True, stateful=True)(x)
        if opt.dropout > 0:
