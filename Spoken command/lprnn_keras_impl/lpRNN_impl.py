...
class lpRNNCell(keras.layers.SimpleRNNCell):
    ...
    """
        def __init__(self, units, 
                     ...
                     noise=0.2,
                     **kwargs):
            super(lpRNNCell, self).__init__(units,
                                         activation='tanh',
                                         use_bias=True,
                                         kernel_initializer='glorot_uniform',
                                         recurrent_initializer='orthogonal',
...
#                                                          initializer=keras.initializers.RandomUniform(minval=0.1,maxval=0.99))
                self._retention_ratio_val = self.add_weight('ret_ratio', 
                                                          shape=[self._num_units], 
                                                          initializer=keras.initializers.RandomUniform(minval=0.1,maxval=0.99))
                self._retention_ratio = keras.backend.sigmoid(self._retention_ratio_val)
            else:    
            ...
            super(lpRNNCell,self).build(input_shape)
                            
        def call(self, inputs, states, training=None):
            prev_output = states[0]

            if self.noise > 0:
                N = K.random_normal(tf.shape(inputs), mean=0.0, stddev=self.noise)
                inputs = inputs + N
            ...
            if (0 < self.recurrent_dropout < 1 and
                ...
                self._recurrent_dropout_mask = _generate_dropout_mask(K.ones_like(prev_output),
            ...
            output = self._retention_ratio * prev_output + (1 - self._retention_ratio)*output
...
class lpRNN(keras.layers.RNN):
    ...
    def __init__(self, units,
        ...
        super(lpRNN, self).__init__(cell,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        ...
        return super(lpRNN, self).call(inputs,
                                       mask=mask,
                                       training=training,
    ...
    def get_config(self):
        ...
        base_config = super(lpRNN, self).get_config()
